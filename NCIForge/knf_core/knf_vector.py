...
def assemble_knf_vector(
    ...
    f6: Optional[int], f7: Optional[float], f8: Optional[float], f9: Optional[float]
) -> list[float | None]:
    """
    Assembles the 9D KNF vector.
    Order: [f1, f2, f3, f4, f5, f6, f7, f8, f9]
    """
    return [f1, f2, f3, f4, f5, None if f6 is None else float(f6), f7, f8, f9]
...
def write_output_txt(filepath: str, result: KNFResult):
    ...
    with open(filepath, 'w') as f:
        ...
        f.write("=========================\n\n")
        f.write(f"SNCI_raw:       {_format_optional_float(result.SNCI, 6)}\n")
        ...
        f.write(f"f1 (COM Dist):  {vec[0]:.6f} A\n")
        if f2_defined == 0:
            reason = metadata.get("f2_undefined_reason", "undefined")
            f.write(f"f2 (HB Angle):  n/a ({reason})\n")
        else:
            f.write(f"f2 (HB Angle):  {vec[1]:.6f} deg\n")
        f.write(f"f3 (Max Inter WBO):   {vec[2]:.6f}\n")
        f.write(f"f4 (Dipole):    {vec[3]:.6f} D\n")
        f.write(f"f5 (Pol):       {_format_optional_float(vec[4], 6)} au\n")
        f.write(f"f6 (NCI Count): {_format_optional_float(vec[5], 0)}\n")
        f.write(f"f7 (NCI Mean):  {_format_optional_float(vec[6], 6)}\n")
        f.write(f"f8 (NCI Std):   {_format_optional_float(vec[7], 6)}\n")
        f.write(f"f9 (NCI Skew):  {_format_optional_float(vec[8], 6)}\n")
