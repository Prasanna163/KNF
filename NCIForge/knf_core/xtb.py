...
def parse_xtb_log(log_path: str, require_polarizability: bool = True) -> dict:
    ...
    if not os.path.exists(log_path):
        logging.warning("xTB log file not found: %s; returning default xTB metrics", log_path)
        return {
            "f4": 0.0,
            "f5": None,
            "f5_available": False,
            "f5_unavailable_reason": "log_file_not_found",
            "f3": 0.0,
        }
    ...
    if dipole_val is None:
        logging.warning("Dipole moment (f4) not found in xTB log (%s); defaulting to f4=0.0", log_path)
        dipole_val = 0.0
