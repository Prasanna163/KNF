...
def resolve_cpu_backend_when_torch_missing(
    ...
    *,
    ...
    # CPU mode: if torch is missing, attempt auto-installation before giving up.
    try:
        logging.info("PyTorch missing for CPU backend; installing torch...")
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "torch", "--extra-index-url", "https://download.pytorch.org/whl/cpu"],
            capture_output=True,
            text=True,
            timeout=300,
        )
        ...
        logging.warning("Auto-installing torch failed: %s", exc)

    # CPU mode: if torch is still missing, auto-fallback to Multiwfn CPU backend.
