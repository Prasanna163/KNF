...
def compute_scdi_metrics(
    ...
    if not os.path.exists(cosmo_path):
        ...
        return SCDIMetrics(variance=None, scdi=None, valid=False, reason="cosmo_file_not_found")

    rows = _extract_segment_rows(cosmo_path)
    if not rows:
        return SCDIMetrics(variance=None, scdi=None, valid=False, reason="no_segment_rows")
    ...
    if not np.any(mask):
        return SCDIMetrics(variance=None, scdi=None, valid=False, reason="no_valid_area_weights")
...
def compute_scdi(
    ...
    if metrics.scdi is not None:
        return metrics.scdi
    return 0.0 if metrics.variance is None else metrics.variance
