...
def _tool_names(tool: str) -> tuple[str, ...]:
    if tool == "xtb":
        return ("xtb.exe", "xtb-cpu.exe", "xtb")
    if tool == "obabel":
        return ("obabel.exe", "obabel")
    return (f"{tool}.exe", tool)


def _is_executable_runnable(exe_path: str) -> bool:
    try:
        proc = subprocess.run(
            [exe_path, "--version"],
            capture_output=True,
        ...
        if proc.returncode in (3221225595, -1073741515, 0xC0000135, 127):
            log_tool_debug(f"_is_executable_runnable: '{exe_path}' crashed with exit code {proc.returncode} (missing DLLs or invalid binary)")
            return False
        return True
    except Exception as exc:
        log_tool_debug(f"_is_executable_runnable: '{exe_path}' probe failed: {exc}")
        return False
...
def _resolve_candidate_executable(candidate_path: str, names: tuple[str, ...]) -> Optional[str]:
    ...
    if candidate.is_file():
        if candidate.name.lower() in {n.lower() for n in names}:
            exe_str = str(candidate)
            if _is_executable_runnable(exe_str):
                return exe_str
        return None
    if candidate.is_dir():
        for name in names:
...
def _common_external_tool_candidates(tool: str) -> list[str]:
    ...
    if tool == "xtb":
        knf_core_dir = Path(__file__).resolve().parent
        nciforge_root = knf_core_dir.parent
        candidates.extend(
            [
                str(nciforge_root / "nciforge_xtbx" / "runtime" / "xtb-win-release" / "bin"),
                str(nciforge_root / "nciforge_xtbx" / "runtime" / "xtb-win-release"),
                str(nciforge_root / "tools" / "xtb" / "bin"),
                str(Path.cwd() / "nciforge_xtbx" / "runtime" / "xtb-win-release" / "bin"),
                str(Path.cwd() / "NCIForge" / "nciforge_xtbx" / "runtime" / "xtb-win-release" / "bin"),
            ]
        )
    if tool == "obabel":
...
def register_external_tool_path(tool: str, candidate_path: str, persist: bool = True) -> Optional[str]:
    ...
    if tool == "xtb":
        params_dir = Path(exe).parent / "params"
        if not params_dir.exists():
            params_dir = Path(exe).parent.parent / "params"
        if params_dir.exists():
            os.environ["XTBPATH"] = str(params_dir)
            log_tool_debug(f"register_external_tool_path: set XTBPATH={params_dir}")
...
def log_tool_debug(msg: str) -> None:
    path = r"D:\Prasubby\NCIForge\nciforge_debug.log"
    try:
        with open(path, "a", encoding="utf-8") as f:
            f.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {msg}\n")
    except Exception:
        pass
    print(f"[TOOL_DEBUG] {msg}", file=sys.stderr)
...
def ensure_external_tools_in_path(persist: bool = False) -> dict[str, Optional[str]]:
    ...
    for tool in ("xtb", "obabel"):
        ...
        if current:
            ...
            log_tool_debug(f"ensure_external_tools_in_path('{tool}'): already in PATH at {current}")
            continue
        names = _tool_names(tool)
        candidates = _common_external_tool_candidates(tool)
        for candidate in candidates:
            exe = _resolve_candidate_executable(candidate, names)
            if not exe:
                continue
            resolved[tool] = register_external_tool_path(tool, exe, persist=persist)
            log_tool_debug(f"ensure_external_tools_in_path('{tool}'): registered {exe}")
            if resolved[tool]:
                break
        if not resolved[tool]:
            log_tool_debug(f"ensure_external_tools_in_path('{tool}'): failed to find tool in candidates {candidates}")
    return resolved
...
def resolve_external_tool_command(tool: str) -> Optional[str]:
    ...
    current = shutil.which(tool)
    if current:
        log_tool_debug(f"resolve_external_tool_command('{tool}'): found in PATH -> {current}")
        return current
    candidates = _common_external_tool_candidates(tool)
    for candidate in candidates:
        exe = _resolve_candidate_executable(candidate, names)
        if exe:
            log_tool_debug(f"resolve_external_tool_command('{tool}'): found candidate -> {exe}")
            return exe
    log_tool_debug(f"resolve_external_tool_command('{tool}'): NOT FOUND in candidates {candidates}")
    return None
    return None
