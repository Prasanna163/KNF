...
def run_xtb_opt(
    ...
    if return_code != 0:
        if os.path.exists(output):
            ...
            return output
        logging.warning(
            "xTB optimization failed (exit code %s). Falling back to input geometry %s.",
            return_code,
            filepath,
        )
        try:
        ...
        if progress_callback is None:
            with open(xtb_log, 'w', encoding='utf-8', errors='replace') as log:
                try:
                    ...
                    logging.warning(f"xTB SP command exited with error: {e}. Proceeding without xTB SP output.")
        else:
            return_code = _run_xtb_streaming(
                cmd,
                cwd=cwd,
            ...
            if return_code != 0:
                logging.warning(f"xTB SP command exited with return code {return_code}. Proceeding without xTB SP output.")
