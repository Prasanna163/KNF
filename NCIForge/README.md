...
> **Using the integrated KNF Studio checkout?** Start from the parent
> `NCI-FORGE-F-F` repository with `.\run.ps1`. Do not run global Uvicorn from
> the `MAGGIE` folder; other editable projects on this machine also expose
> `knf_core`. See the parent [`README.md`](../README.md) for the verified app
> workflow.
...
Or, from an activated NCIForge environment, run Uvicorn directly:

```bash
python -m uvicorn knf_core.api:app --host 0.0.0.0 --port 8000
```
