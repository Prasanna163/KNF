...
def resolve_bash() -> str:
    ...
    raise XtbxUnavailable(
        "MSYS2/Git bash was not found. Install MSYS2 or Git for Windows, or set XTBX_BASH to bash.exe."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    env["PYTHON"] = windows_to_msys_path(Path(sys.executable))

    script = package_dir() / "xtbx_run.sh"
    script_path = windows_to_msys_path(script) if os.name == "nt" else str(script)
    cmd = [bash, script_path, *(_normalize_arg(arg) for arg in args)]
